...
def import_na28( collection, book_name, na28_html_file ):
    ...
    with open(na28_html_file, 'r') as f:
        ...
        text = text.replace('</p>\n<p class="p4"><span class="s4">¦</span>', '</p><p class="p4"><span class="s4">¦</span>')
        text = text.replace('<span class="s2">⸀</span> θεω <i>et</i></p>\n<p class="p3"><span class="s2">⸁</span> Χριστω', '⸀ θεω et ⸁ Χριστω')
        ...
        text = text.replace('[<span class="s4">:</span> , <i>et</i> <span class="s4"><sup>:</sup></span><sup>1</sup> −]</p>\n<p class="p3">',"")

        text = re.sub(r"\(<i>cf</i>.*?\)", "", text)


        text = re.sub(r"(.)<sup>\(</sup>\*<sup>\).([c\d])</sup>", r"\1* \1<sup>\2</sup>", text)
        ...
        for verse in verses:
            ...
            current_verse_start_obj = BibleVerse.get_from_string( f"{book_name} {current_chapter}:{current_verse}")
            if current_verse_end and current_verse_end != current_verse:
                current_verse_end_obj = BibleVerse.get_from_string( f"{book_name} {current_chapter}:{current_verse_end}")
            else:
            ...
            for variation_unit_index, variation_unit in enumerate(variation_units):
                ...
                print(f"\t--variation unit {current_chapter}:{current_verse}. {variation_unit_index}")
                ...
                if "A B 1739 <i>txt</i>" in variation_unit:
                    variation_unit = variation_unit.replace("A B 1739 <i>txt</i>", "A B 1739 ¦</span> <i>txt</i>")
                ...
                for reading_app in readings:
                    ...
                    if reading == None:
                        if reading_app[0] == "°":
                        ...
                        elif m := re.match(r"(.*?[[α-ωἤῶ][α-ωἤῶ ]*<i>.+?<\/i>)(.*)", reading_app):
                            reading = m.group(1)
                            reading_app = m.group(2)    
                            reading_found = "Greek with italics"      
                        elif m := re.match(r"(.*?[[α-ωἤῶΧ][α-ωἤῶΧ ]*\([α-ωἤΧῶ\+\−].+?\)[α-ωἤΧῶ ]*)(.*)", reading_app):
                            reading = m.group(1)
                            reading_app = m.group(2)    
                            reading_found = "Greek with brackets"      
                        elif m := regex.match(r"(.*[α-ωἤῶ])(.*)", reading_app, re.DOTALL):
                            reading = m.group(1)
                            reading_app = m.group(2)
                            reading_found = "Greek"      
                        elif m := re.match(r"(<span class=\"s2\">[⸉⸂⸄].*?<\/i>)(.*)", reading_app):
                            reading = m.group(1)
                            reading_app = m.group(2)
                            reading_found = "siglum with italics"      
                        elif m := re.match(r"(<p class=\"p3\"><span class=\"s2\">⸄<\/span> .*</i>)(.*)", reading_app):
                            reading = m.group(1)
                            reading_app = m.group(2)
                            ...
                            reading_app = m.group(1)
                            reading_found = "sigla"
                        elif m:= re.match(r'<span class="s2">⸂<\/span> (<i>.*<\/i>)(.*)', reading_app):
                            reading = m.group(1)
                            reading_app = m.group(2)
                    ...
                    for disallowed_reading in disallowed_reading_context:
                        if disallowed_reading in reading:
                            raise Exception(f"Disallowed reading: {reading}. reading found {reading_found}")

                    if "(" in reading and ")" not in reading:
                        raise Exception(f"Disallowed reading: {reading}. Brackets uneven. reading found {reading_found}")

                    current_reading_obj, _ = Reading.objects.update_or_create( text=reading, location=current_location )
                    ...
                    reading_app = reading_app.replace(" <i>pm;</i>", "")
                    reading_app = reading_app.replace(" vid</sup>", "</sup><sup>vid</sup>")
                    reading_app = reading_app.replace(" pt</sup>", "</sup><sup>pt</sup>")
                    reading_app = reading_app.replace(" v.l.</sup>", "</sup><sup>v.l.</sup>")
                    ...
                    reading_app = re.sub(r'\(<span class="s2">⸉<\/span>(.*?)\)', r"\1", reading_app )

                    reading_app = reading_app.replace("Ir<sup>arm, lat pt</sup>", "Ir__arm_lat_pt" )
                    reading_app = reading_app.replace("Ir<sup>arm, lat</sup><sup>pt</sup>", "Ir__arm_lat_pt" )
                    reading_app = reading_app.replace("Ir<sup>gr, lat</sup><sup>pt</sup>", "Ir__gr_lat_pt" )

                    reading_app = reading_app.replace("Ophites<sup>Ir lat</sup>", "Ophites<sup>Ir_lat</sup>" )

                    reading_app = re.sub(r'sy<sup>(h|p|ph|hmg|h\*\*)\.(h|p|ph|hmg|h\*\*)<\/sup>', r'sy__\1 sy__\2', reading_app)
                    ...
                    for witness in witnesses:
                        ...
                        witness_obj, _ = SiglumWitness.objects.update_or_create(siglum=witness)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('BOOK_NAME', type=str, help="The name of the biblical book being imported.")
        parser.add_argument('NA28_HTML_FILE', type=str, help="An HTML file taken from the NA28 apparatus in Logos.")
        parser.add_argument('--flush', type=str, help="If 'yes', then it removes old locations in the collection before importing.")

    def handle(self, *args, **options):
        na28_html_file = options["NA28_HTML_FILE"]

        collection, _ = Collection.objects.update_or_create( name=os.path.basename(na28_html_file) )
