...
class WitnessBase(PolymorphicModel):
    ...
    def attestations_at(self, location, corrector=None):
        return Attestation.objects.filter(
            witness=self, reading__location=location, corrector=corrector
        )

    def reading_ids_at(self, location, corrector=None):
        return {
            attestation.reading.id
            for attestation in self.attestations_at(location, corrector)
        }
    ...
    def locations_attested(self, collection, corrector=None):
        location_ids = (
            Attestation.objects.filter(
                witness=self,
                reading__location__collection=collection,
                corrector=corrector,
            )
            .values_list("reading__location__id", flat=True)
            .distinct()
        )
    ...
    def agreement_array(self, other_witness, locations):
        ...
        for index, location in enumerate(locations):
            ...
            if my_readings == other_readings:
                ...
                len(my_readings.intersection(other_readings)) > 0
                or len(my_readings) == 0
                or len(other_readings) == 0
            ):
                results[index] = 1
            else:
...
class Collection(models.Model):
    ...
    def witnessess_no_correctors(self):
        witnesses = WitnessBase.objects.filter(
            attestation__reading__location__collection=self, attestation__corrector=None
        )
    ...
    def write_nexus(self, filename, witnesses=None, locations=None, min_locations=1, exclude_sigla=None):
        ...
        state_counts = np.asarray(
            [location.reading_set.count() for location in locations]
        )
            ...
            for witness in witnesses:
                ...
                for location in locations:
                    ...
                    if attestations.count() == 0:
                        ...
                        labels = [
                            location.label_for_reading(attestation.reading)
    ...
    def write_beast_patch(
        self,
        beast_patch_filename,
        locations=None,
        atext_probability=0.6,
        only_category_A=False,
        clock_name="StrictClock",
    ):
        collection = self
        if locations is None:
        ...
        with open(beast_patch_filename, "w") as file:
            ...
            for location in locations:
                ...
                file.write(
                    f'<distribution id="morphTreeLikelihood.{family}.character{index}" spec="TreeLikelihood" useAmbiguities="true" branchRateModel="@{clock_name}" tree="@Tree.t:{family}">\n'
                )
                file.write(f"\t<data\n")
                ...
                file.write(
                    f'\t\t<userDataType id="morphDataType.{family}.character{index}" spec="beast.evolution.datatype.StandardData" ambiguities="REPLACE_AMBIGUTIIES_HERE" nrOfStates="{state_count}"/>\n'
                )

                file.write(f"\t</data>\n")

                file.write(
                    f'\t<siteModel id="morphSiteModel.s:{family}.character{index}" spec="SiteModel">\n'
                )

                file.write(
                    f'\t\t<parameter id="mutationRate.s:.{family}.character{index}" spec="parameter.RealParameter" estimate="false" name="mutationRate">1.0</parameter>\n'
                )

                file.write(
                    f'\t\t<parameter id="gammaShape.s:.{family}.character{index}" spec="parameter.RealParameter" estimate="false" name="shape">1.0</parameter>\n'
                )

                # file.write(f'\t\t<substModel id="LewisMK.s:.{family}.character{index}" spec="LewisMK" datatype="@morphDataType.{family}.character{index}"/>\n')
                file.write(
                    f'\t\t<substModel id="SubstModel.{family}.character{index}" spec="GeneralSubstitutionModel">\n'
                )

                file.write(
                    f'\t\t\t<frequencies id="freqs.{family}.character{index}" spec="Frequencies">\n'
                )
                file.write(
                    f'\t\t\t\t<frequencies id="frequencies.{family}.character{index}" spec="parameter.RealParameter" value="{freq_string}" estimate="false"/>\n'
                )
                file.write(f"\t\t\t</frequencies>\n")

                file.write(
                    f'\t\t\t<parameter id="rates.{family}.character{index}" spec="parameter.CompoundValuable" name="rates">\n'
                )
                ...
                if location.ausgangstext and (
                    only_category_A or location.category == location.CategoryUBS.A
                ):
                    not_atext_probability = (1.0 - root_probability) / (
                        location.reading_set.count() - 1
                    )

                    not_atext_probability_str = str(not_atext_probability)
                    rootfreqs = [
                        str(atext_probability)
                        if location.ausgangstext == reading
                        else not_atext_probability_str
                        for reading in location.reading_set.all()
                    ]
                    rootfreq_string = " ".join(rootfreqs)
                    # print(rootfreq_string, location.reading_set.count())
                    file.write(
                        f'\t<rootFrequencies id="rootfreqs.{family}.character{index}" spec="Frequencies">\n'
                    )
                    file.write(
                        f'\t\t<frequencies id="rootfrequencies.{family}.character{index}" spec="parameter.RealParameter" value="{rootfreq_string}" estimate="false"/>\n'
                    )
    ...
    def write_carlson(self, file, include_ubs=False):
        ...
        for location in self.locations:
            ...
            file.write(
                "\t| %s \n"
                % " ".join(
                    [format_reading(reading.text) for reading in location.readings[1:]]
                )
            )
            file.write("]\n")
            ...
            for reading_index, reading in enumerate(location.readings):
                original_witnesses_names = [
                    name
                    for name in reading.original_witness_names()
                    if location_witnesses_counter[name] == 1
                ]
...
class LocationBase(PolymorphicModel):
    start_verse = models.ForeignKey(
        Verse, on_delete=models.CASCADE, related_name="variant_start_verse"
    )
    ...
    def reference(self, abbreviation=False):
        return self.start_verse.reference(
            abbreviation=abbreviation, end_verse=self.end_verse
        )
    ...
    def next(self):
        return (
            type(self)
            .objects.filter(collection=self.collection, rank__gt=self.rank)
            .first()
        )
    ...
    def prev(self):
        return (
            type(self)
            .objects.filter(collection=self.collection, rank__lt=self.rank)
            .last()
        )
    ...
    def verses(self):
        ...
        return verse_class.objects.filter(
            rank__gte=self.start_verse.rank, rank__lte=end_verse.rank
        )
...
class LocationUBS(LocationBase):
    ausgangstext = models.ForeignKey(
        "Reading",
        on_delete=models.SET_NULL,
        default=None,
        blank=True,
    ...
    category = models.IntegerField(
        choices=CategoryUBS.choices, default=None, blank=True, null=True
    )
