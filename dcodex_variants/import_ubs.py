...
def pop_witnesses_class(
    current_reading, witnesses_string, members, class_name, general_subvariant=""
):
    parentheses = False
    ...
    if m:
        ...
        for member_name in members_string.split():
            ...
            if class_name == "𝔓":
                ...
                member_name = (
                    "P" + member_name
                )  # Make it a normal P because it is inline
            else:
...
def pop_all_classes(current_reading, text, witnesses, subvariant=""):
    ...
    text, witnesses = pop_witnesses_class(
        current_reading, text, witnesses, "𝔓", subvariant
    )
    text, witnesses = pop_witnesses_class(
        current_reading, text, witnesses, "it", subvariant
    )
    text, witnesses = pop_witnesses_class(
        current_reading, text, witnesses, "cop", subvariant
    )
    text, witnesses = pop_witnesses_class(
        current_reading, text, witnesses, "syr", subvariant
    )
...
def deal_with_sups(current_reading, text, witnesses, subvariant=""):
    ...
    for superscripted in re.findall(superscripted_pattern, text):
        ...
        if superscript.isdigit() or superscript == "c":
            if corrector == "*":
                add_attestation(
                    current_reading, name, corrector=corrector, subvariant=subvariant
                )  # e.g. D*<sup>, 2</sup>

            add_attestation(
                current_reading, name, corrector=superscript, subvariant=subvariant
            )
        else:
            add_attestation(
                current_reading,
...
def split_witnesses(current_reading, text, subvariant=""):
    ...
    for name in text.split():
        ...
        add_attestation(
            current_reading, name, subvariant=subvariant, corrector=corrector, info=info
        )
    return witnesses
...
def strip_commas_from_sups(text):
    ...
    while True:
        ...
        new_string += text[start : start + left] + text[
...
def parse_witnesses(current_reading, witnesses_string, members):
    ...
    while witnesses_string:
        ...
        if (
            first_space >= 0
            and (first_space < first_sup or first_sup < 0)
            and (first_space < first_bracket or first_bracket < 0)
        ):  # Check if space
            ...
            sups_string, members = pop_all_classes(
                current_reading, sups_string, members
            )
            ...
            for component in components:
                ...
                members += split_witnesses(
                    current_reading, component, subvariant
                )  # This should be a call to this function

            witnesses_string = (
                witnesses_string[:left_bracket] + witnesses_string[right_bracket + 1 :]
            )
        else:  # End of line
            if witnesses_string:
...
def pop_brackets(witnesses_string, members):
    ...
    while 0 <= left_bracket < right_bracket:
        ...
        if witnesses_string[:left_bracket].rfind("<sup>") > witnesses_string[
            :left_bracket
        ].rfind("</sup>"):
            ...
            witnesses_string, members = pop_all_classes(
                current_reading, witnesses_string, members
            )
            witnesses_string, members = deal_with_sups(
                current_reading, witnesses_string, members
            )
...
def import_reading_from_html(current_location, reading):
    ...
    if m:
        ...
        current_reading, _ = Reading.objects.update_or_create(
            text=reading_text, location=current_location
        )
        ...
        witnesses_string = witnesses_string.replace(
            ",</sup> <sup>ach</sup>2", ", ach2</sup>"
        )
        witnesses_string = witnesses_string.replace("bo</sup>pt<sup>,</sup>", "bo|pt, ")
        witnesses_string = witnesses_string.replace(
            "bo</sup>ms<sup>)</sup>", "bo|ms</sup>)"
        )
        witnesses_string = witnesses_string.replace(
            "2<sup>nd</sup> epistle of Clement", "2nd_epistle_of_Clement"
        )
        ...
        witnesses_string = witnesses_string.replace(
            "ach2</sup>?", "ach2|Uncertain</sup>"
        )

        witnesses_string = witnesses_string.replace("bo</sup>?", "bo|Uncertain</sup>")

        witnesses_string = witnesses_string.replace(
            "ach</sup>2?", "ach2|Uncertain</sup>"
        )

        witnesses_string = witnesses_string.replace(
            "r</sup>1<sup> vid</sup>", "r1|vid</sup>"
        )
        ...
        witnesses_string = witnesses_string.replace(
            "r</sup>1<sup>?</sup>", "r1|Uncertain</sup>"
        )

        witnesses_string = witnesses_string.replace(
            " r</sup>1<sup>)</sup>", " r1</sup>)"
        )
        ...
        witnesses_string = re.sub(
            r"<span class=\"s1\">𝔓<\/span>", r"𝔓", witnesses_string
        )
        witnesses_string = re.sub(
            r"<span class=\"s2\">𝔓<\/span>", r"𝔓", witnesses_string
        )
        ...
        witnesses_string = regex.sub(
            r"<\/sup>([^\s\p{Punct}]+)<sup>", r"|\1", witnesses_string
        )
        witnesses_string = regex.sub(
            r"<\/sup>([^\s\p{Punct}]+)", r"|\1</sup>", witnesses_string
        )
        witnesses_string = witnesses_string.replace("</sup>, <sup>", " ")
        witnesses_string = witnesses_string.replace(
            "h with</sup> *<sup>", "hmetobelos "
        )
        witnesses_string = re.sub(
            "h with</sup>\s*\*", "hmetobelos</sup>", witnesses_string
        )
        witnesses_string = witnesses_string.replace("</sup>*", "*</sup>")
        witnesses_string = witnesses_string.replace(
            "Letter of Hymenaeus", "Letter-of-Hymenaeus"
        )
        witnesses_string = witnesses_string.replace(
            "Greek and Latin mss", "Greek-mss Latin-mss"
        )
        ...
        witnesses_string = witnesses_string.replace(
            "Docetists and Naassenes<sup>acc. to Hippolytus</sup>",
            "Docetists<sup>acc. to Hippolytus</sup>  Naassenes<sup>acc. to Hippolytus</sup>",
        )
        witnesses_string = re.sub(
            r"<i>l</i><sup>AD</sup>", r"𝑙-Apostoliki-Diakonia", witnesses_string
        )
        witnesses_string = re.sub(
            r"<i>l</i><sup>AD1/2</sup>",
            r"𝑙-Apostoliki-Diakonia<sup>1/2</sup>",
            witnesses_string,
        )
        ...
        witnesses_string = witnesses_string.replace(
            "<sup>bo|ms</sup>_mg", "<sup>bo|ms_mg</sup>"
        )
        ...
        witnesses_string = re.sub(
            r"(\(it<sup>.*)r1\)<\/sup>", r"\1r1</sup>)", witnesses_string
        )  # Hack for John

        members = []

        witnesses_string, members = parse_witnesses(
            current_reading, witnesses_string, members
        )
